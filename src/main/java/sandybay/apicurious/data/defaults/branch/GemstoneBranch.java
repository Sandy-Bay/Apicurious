...
public class GemstoneBranch
{
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.EMERALDINE.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.EMERALDINE.species(), "emeraldine").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.EMERALDINE)).withProductionData(builder -> builder.withFertility(Fertility.LOW_FERTILITY).withLifespan(Lifespan.SHORT).withPollination(Pollination.SLOWEST).withSpeed(Speed.SLOWEST).withWorkCycle(Workcycle.ALWAYS)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.STONE).withHumidityTolerance(HumidityTolerance.LOW_TOLERANCE).withTemperatureTolerance(TemperatureTolerance.LOW_TOLERANCE).ignoresSky().ignoresRain()).withOutputData(builder -> builder.withTable(ApicuriousSpecies.EMERALDINE.output())).recessive().build());
  }
