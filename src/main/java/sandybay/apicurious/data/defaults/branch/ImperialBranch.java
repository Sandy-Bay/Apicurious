...
public class ImperialBranch
{
  ...
  public static void outputsDefaults(BootstrapContext<OutputTable> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.IMPERIAL.output(), OutputTableDefaults.custom().withPool(pool -> pool.when(new ChanceCondition(0.2f)).withResult(result -> result.withResult(ItemRegistrar.DRIPPING_COMB.comb().get()))).withPool(pool -> pool.when(new ChanceCondition(0.15f)).withResult(result -> result.withResult(ItemRegistrar.ROYAL_JELLY.item().get()))).build());
  }
}
