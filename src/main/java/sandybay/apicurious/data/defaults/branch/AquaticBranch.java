...
public class AquaticBranch
{
  ...
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.OCEAN.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.OCEAN.species(), "ocean").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.OCEAN)).withProductionData(builder -> builder.withLifespan(Lifespan.SHORTER).withPollination(Pollination.SLOW).withSpeed(Speed.SLOWEST)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.LILY_PAD).withHumidityPreference(HumidityPreference.DAMP).withHumidityTolerance(HumidityTolerance.LOW_TOLERANCE).ignoresRain()).withOutputData(builder -> builder.withTable(ApicuriousSpecies.OCEAN.output())).build());
    bootstrap.register(ApicuriousSpecies.STAINED.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.STAINED.species(), "stained").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.STAINED)).withProductionData(builder -> builder.withLifespan(Lifespan.SHORTER).withPollination(Pollination.SLOW).withSpeed(Speed.SLOWEST)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.LILY_PAD).withHumidityPreference(HumidityPreference.DAMP).withHumidityTolerance(HumidityTolerance.LOW_TOLERANCE).ignoresRain()).withOutputData(builder -> builder.withTable(ApicuriousSpecies.WATER.output())).build());
  }
