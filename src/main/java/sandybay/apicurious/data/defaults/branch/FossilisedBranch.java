...
public class FossilisedBranch
{
  ...
  {
    bootstrap.register(ApicuriousSpecies.FOSSILISED.species(),
    ...
    bootstrap.register(ApicuriousSpecies.FORGOTTEN.species(),
  ...
  {
    bootstrap.register(ApicuriousSpecies.FOSSILISED.mutation(),
            mutation(bootstrap)
                    .withFirst(ApicuriousSpecies.PRIMEVAL.species())
                    .withSecond(ApicuriousSpecies.FARMERLY.species())
                    .withChance(0.08f)
                    .withOutput(ApicuriousSpecies.FOSSILISED.species())
                    .build()
    );
    bootstrap.register(ApicuriousSpecies.FORGOTTEN.mutation(),
            mutation(bootstrap)
                    .withFirst(ApicuriousSpecies.FOSSILISED.species())
                    .withSecond(ApicuriousSpecies.DEMONIC.species())
                    .withChance(0.05f)
                    .withOutput(ApicuriousSpecies.FORGOTTEN.species())
                    .build()
    );
  }
  ...
  {
    bootstrap.register(ApicuriousSpecies.FOSSILISED.output(), OutputTableDefaults.custom()
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.2f))
                    .withResult(result -> result.withResult(ItemRegistrar.ANCIENT_COMB.get()))
            )
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.02f))
                    .withResult(result -> result.withResult(ItemRegistrar.FOSSILISED_COMB.get()))
            ).build()
    );
    bootstrap.register(ApicuriousSpecies.FORGOTTEN.output(), OutputTableDefaults.custom()
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.2f))
                    .withResult(result -> result.withResult(ItemRegistrar.ANCIENT_COMB.get()))
            )
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.02f))
                    .withResult(result -> result.withResult(Items.ANCIENT_DEBRIS))
            ).build()
    );
  }
}
