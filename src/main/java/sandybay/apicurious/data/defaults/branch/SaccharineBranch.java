...
public class SaccharineBranch
{
  ...
  {
    bootstrap.register(ApicuriousSpecies.SWEETENED.species(),
            SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.SWEETENED.species(), "sweetened")
                    .withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.SWEETENED))
                    .withProductionData(builder -> builder
                            .withLifespan(Lifespan.SHORTER)
                            .withPollination(Pollination.FASTER)
                            .withSpeed(Speed.SLOWER))
                    .withEnvironmentalData(builder -> builder.withFlowers(Flowers.SUGAR_CANE))
                    .withOutputData(builder -> builder.withTable(ApicuriousSpecies.SWEETENED.output()))
                    .build()
    );
    bootstrap.register(ApicuriousSpecies.SUGARY.species(),
            SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.SUGARY.species(), "sugary")
                    .withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.SUGARY))
                    .withProductionData(builder -> builder
                            .withLifespan(Lifespan.SHORTER)
                            .withPollination(Pollination.FASTER)
                            .withSpeed(Speed.SLOWER))
                    .withEnvironmentalData(builder -> builder.withFlowers(Flowers.SUGAR_CANE))
                    .withOutputData(builder -> builder.withTable(ApicuriousSpecies.SUGARY.output()))
                    .build()
    );
  }
  ...
  {
    bootstrap.register(ApicuriousSpecies.SWEETENED.mutation(),
            mutation(bootstrap)
                    .withFirst(ApicuriousSpecies.VALIANT.species())
                    .withSecond(ApicuriousSpecies.DILIGENT.species())
                    .withChance(0.15f)
                    .withOutput(ApicuriousSpecies.SWEETENED.species())
                    .build()
    );
    bootstrap.register(ApicuriousSpecies.SUGARY.mutation(),
            mutation(bootstrap)
                    .withFirst(ApicuriousSpecies.SWEETENED.species())
                    .withSecond(ApicuriousSpecies.RURAL.species())
                    .withChance(0.15f)
                    .withOutput(ApicuriousSpecies.SUGARY.species())
                    .build()
    );
  }
  ...
  {
    bootstrap.register(ApicuriousSpecies.SWEETENED.output(), OutputTableDefaults.custom()
            .withPool(pool -> pool
    ...
    bootstrap.register(ApicuriousSpecies.SUGARY.output(), OutputTableDefaults.custom()
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.4f))
                    .withResult(result -> result.withResult(Items.HONEYCOMB))
            )
