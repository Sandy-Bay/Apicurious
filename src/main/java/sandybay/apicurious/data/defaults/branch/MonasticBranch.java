...
public class MonasticBranch
{
  ...
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.HERMITIC.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.HERMITIC.species(), "hermitic").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.HERMITIC)).withProductionData(builder -> builder.withFertility(Fertility.LOW_FERTILITY).withLifespan(Lifespan.LONG).withPollination(Pollination.FASTEST).withSpeed(Speed.SLOWER)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.WHEAT).withHumidityTolerance(HumidityTolerance.LOWEST_TOLERANCE).withTemperatureTolerance(TemperatureTolerance.LOWEST_TOLERANCE)).withOutputData(builder -> builder.withTable(ApicuriousSpecies.HERMITIC.output())).recessive().build());
  }
