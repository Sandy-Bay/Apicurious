...
public class FrozenBranch
{
  ...
  public static void outputsDefaults(BootstrapContext<OutputTable> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.ICY.output(), OutputTableDefaults.custom().withPool(pool -> pool.when(new ChanceCondition(0.2f)).withResult(result -> result.withResult(ItemRegistrar.ICE_SHARD.item().get()))).withPool(pool -> pool.when(new ChanceCondition(0.2f)).withResult(result -> result.withResult(ItemRegistrar.FROZEN_COMB.comb().get()))).build());
    bootstrap.register(ApicuriousSpecies.GLACIAL.output(), OutputTableDefaults.custom().withPool(pool -> pool.when(new ChanceCondition(0.4f)).withResult(result -> result.withResult(ItemRegistrar.ICE_SHARD.item().get()))).withPool(pool -> pool.when(new ChanceCondition(0.2f)).withResult(result -> result.withResult(ItemRegistrar.FROZEN_COMB.comb().get()))).build());
