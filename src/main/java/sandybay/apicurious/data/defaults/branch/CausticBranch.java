...
public class CausticBranch
{
  ...
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.ACIDIC.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.ACIDIC.species(), "acidic").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.ACIDIC)).withProductionData(builder -> builder.withLifespan(Lifespan.SHORT).withSpeed(Speed.FAST)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.JUNGLE).withTemperaturePreference(TemperaturePreference.HOT).withTemperatureTolerance(TemperatureTolerance.LOWEST_TOLERANCE).withHumidityPreference(HumidityPreference.DAMP).withHumidityTolerance(HumidityTolerance.LOWEST_TOLERANCE)).withOutputData(builder -> builder.withTable(ApicuriousSpecies.ACIDIC.output())).recessive().build());
  }
