...
public class VirulentBranch
{
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.VIRULENT.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.VIRULENT.species(), "virulent").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.VIRULENT)).withProductionData(builder -> builder.withLifespan(Lifespan.SHORT).withSpeed(Speed.SLOWER)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.JUNGLE).withTemperaturePreference(TemperaturePreference.HOT).withTemperatureTolerance(TemperatureTolerance.LOWEST_TOLERANCE).withHumidityPreference(HumidityPreference.DAMP).withHumidityTolerance(HumidityTolerance.LOWEST_TOLERANCE)).withOutputData(builder -> builder.withTable(ApicuriousSpecies.VIRULENT.output())).recessive().build());
  }
