...
public class TropicalBranch
{
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.EDENIC.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.EDENIC.species(), "edenic").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.EDENIC)).withProductionData(builder -> builder.withLifespan(Lifespan.LONGER).withPollination(Pollination.SLOWEST).withSpeed(Speed.SLOWEST)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.JUNGLE).withHumidityPreference(HumidityPreference.DAMP).withHumidityTolerance(HumidityTolerance.LOWEST_TOLERANCE).withTemperaturePreference(TemperaturePreference.HOT).withTemperatureTolerance(TemperatureTolerance.LOW_TOLERANCE)).withOutputData(builder -> builder.withTable(ApicuriousSpecies.EDENIC.output())).recessive().build());
  }
