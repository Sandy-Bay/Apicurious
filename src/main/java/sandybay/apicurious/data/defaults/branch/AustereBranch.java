...
public class AustereBranch
{
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.HAZARDOUS.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.HAZARDOUS.species(), "hazardous").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.HAZARDOUS)).withProductionData(builder -> builder.withLifespan(Lifespan.LONGER).withPollination(Pollination.SLOWEST).withSpeed(Speed.SLOWEST).withWorkCycle(Workcycle.ALWAYS)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.CACTI).withHumidityPreference(HumidityPreference.ARID).withHumidityTolerance(HumidityTolerance.LOWEST_TOLERANCE).withTemperaturePreference(TemperaturePreference.HOT).withTemperatureTolerance(TemperatureTolerance.LOW_TOLERANCE)).withOutputData(builder -> builder.withTable(ApicuriousSpecies.HAZARDOUS.output())).recessive().build());
  }
