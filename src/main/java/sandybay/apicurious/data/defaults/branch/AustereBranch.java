...
public class AustereBranch
{
  ...
  public static void mutationsDefaults(BootstrapContext<IMutation> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.AUSTERE.mutation(),
            mutation(bootstrap)
                    .withFirst(ApicuriousSpecies.MODEST.species())
                    .withSecond(ApicuriousSpecies.FRUGAL.species())
                    .withChance(0.08f)
                    .withOutput(ApicuriousSpecies.AUSTERE.species())
                    .build()
    );
    bootstrap.register(ApicuriousSpecies.HAZARDOUS.mutation(),
            mutation(bootstrap)
                    .withFirst(ApicuriousSpecies.AUSTERE.species())
                    .withSecond(ApicuriousSpecies.DESOLATE.species())
                    .withChance(0.05f)
  ...
  public static void outputsDefaults(BootstrapContext<OutputTable> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.AUSTERE.output(), OutputTableDefaults.custom()
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.2f))
                    .withResult(result -> result.withResult(ItemRegistrar.PARCHED_COMB.get()))
            )
            .withPool(pool -> pool
                    .when(new ChanceCondition(0.5f))
                    .withResult(result -> result.withResult(ItemRegistrar.POWDERY_COMB.get()))
            ).build()
    );
