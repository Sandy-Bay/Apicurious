...
public class EnergeticBranch
{
  ...
  public static void speciesDefaults(BootstrapContext<IAllele<?>> bootstrap)
  {
    ...
    bootstrap.register(ApicuriousSpecies.ENERGETIC.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.ENERGETIC.species(), "energetic").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.ENERGETIC)).withProductionData(builder -> builder.withLifespan(Lifespan.SHORTER).withPollination(Pollination.SLOWEST).withSpeed(Speed.SLOWEST)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.REDSTONE).ignoresSky()).withOutputData(builder -> builder.withTable(ApicuriousSpecies.EXCITED.output())).build());
    bootstrap.register(ApicuriousSpecies.ECSTATIC.species(), SpeciesDefaults.getSpeciesBuilder(bootstrap, ApicuriousSpecies.ECSTATIC.species(), "ecstatic").withVisualData(builder -> builder.withBeeColor(ApicuriousConstants.ECSTATIC)).withProductionData(builder -> builder.withLifespan(Lifespan.SHORTER).withPollination(Pollination.SLOWEST).withSpeed(Speed.SLOWEST)).withEnvironmentalData(builder -> builder.withFlowers(Flowers.REDSTONE).ignoresSky()).withOutputData(builder -> builder.withTable(ApicuriousSpecies.EXCITED.output())).recessive().build());
  }

  public static void mutationsDefaults(BootstrapContext<IMutation> bootstrap)
  {
    bootstrap.register(ApicuriousSpecies.EXCITED.mutation(), mutation(bootstrap).withFirst(ApicuriousSpecies.CULTIVATED.species()).withSecond(ApicuriousSpecies.VALIANT.species()).withChance(0.1f).withOutput(ApicuriousSpecies.EXCITED.species()).build());
    bootstrap.register(ApicuriousSpecies.ENERGETIC.mutation(), mutation(bootstrap).withFirst(ApicuriousSpecies.EXCITED.species()).withSecond(ApicuriousSpecies.DILIGENT.species()).withChance(0.08f).withOutput(ApicuriousSpecies.ENERGETIC.species()).build());
    bootstrap.register(ApicuriousSpecies.ECSTATIC.mutation(), mutation(bootstrap).withFirst(ApicuriousSpecies.EXCITED.species()).withSecond(ApicuriousSpecies.ENERGETIC.species()).withChance(0.08f).withOutput(ApicuriousSpecies.ECSTATIC.species()).build());
  }
