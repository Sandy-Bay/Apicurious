...
public class CentrifugeMenu extends AbstractContainerMenu
{
  ...
  public CentrifugeMenu(int containerId, Inventory playerInventory, FriendlyByteBuf packetBuffer)
  {
    ...
    addCentrifugeSlots(new ConfigurableItemStackHandler(10).setInputFilter(((stack, slot) -> slot == 0)));
  ...
  private void addCentrifugeSlots(ConfigurableItemStackHandler inventory)
  {
    this.addSlot(new SlotItemHandler(inventory, 0, 44, 34));
    int slot = 1;
    int x = 97;
    int y = 18;
    for (int i = 0; i < 3; i++)
    {
      for (int j = 0; j < 3; j++)
      {
        this.addSlot(new SlotItemHandler(inventory, slot, x + j * 18, y + i * 18));
        slot++;
      }
    }
  }
