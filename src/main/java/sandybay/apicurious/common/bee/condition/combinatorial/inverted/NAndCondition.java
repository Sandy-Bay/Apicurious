...
public record NAndCondition(Holder<ICondition> first, Holder<ICondition> second) implements ICondition
{
  ...
  {
    return first.value().getDisplayText().copy().append(" NAND ").append(second.value().getDisplayText().copy());
  }
}
