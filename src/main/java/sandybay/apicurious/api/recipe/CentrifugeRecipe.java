...
public record CentrifugeRecipe(ItemStack input, int duration, List<CentrifugeOutput> outputs)
{
  public static final Codec<CentrifugeRecipe> CODEC = RecordCodecBuilder.create(instance ->
          instance.group(
                  ItemStack.CODEC.fieldOf("input").forGetter(CentrifugeRecipe::input),
  ...
  public static final StreamCodec<RegistryFriendlyByteBuf, CentrifugeRecipe> STREAM_CODEC = StreamCodec.composite(
          ItemStack.STREAM_CODEC, CentrifugeRecipe::input,
          ByteBufCodecs.INT, CentrifugeRecipe::duration,
  ...
  public static class Builder
  {
    ...
    {
      if (this.outputs.size() > 9) throw new IllegalArgumentException("There can be no more than 9 outputs for any given centrifuge recipe");
    ...
    public CentrifugeRecipe build()
    {
      ...
      if (this.duration <= 0) throw new IllegalArgumentException("Duration must be equal to or above 1 for Centrifuge Recipe");
      if (this.outputs.isEmpty() || this.outputs.size() > 9) throw new IllegalArgumentException("The amount of outputs for any given Centrifuge Recipe should been between 1 and 9 possible outputs");
  ...
  {
    public static final Codec<CentrifugeOutput> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                ItemStack.CODEC.fieldOf("output").forGetter(CentrifugeOutput::output),
                Codec.FLOAT.fieldOf("chance").forGetter(CentrifugeOutput::chance)
            ).apply(instance, CentrifugeOutput::new)
    );
    public static final StreamCodec<RegistryFriendlyByteBuf, CentrifugeOutput> STREAM_CODEC = StreamCodec.composite(
            ItemStack.STREAM_CODEC, CentrifugeOutput::output,
            ByteBufCodecs.FLOAT, CentrifugeOutput::chance,
            CentrifugeOutput::new
    );
  }
